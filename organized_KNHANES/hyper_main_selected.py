...
#auto imputed data - KNN + min max scaler
...
#rev (reduced risk factor) KNN+ standard scaler
'''
...
def h_model(x_train, y_train, x_test, y_test):
    ...
    model.add(Dense({{choice([1024,2048,3000,4096,5000,6000,7000,8000,9000])}}, input_shape=(f_n,), kernel_regularizer='l1'))
    #model.add(Activation('relu'))
    model.add(Activation({{choice(['relu','sigmoid','tanh','selu'])}}))
    model.add(BatchNormalization())
    model.add(Dense({{choice([64,128,256,512,1024,2048,3000,4096,5000,6000,7000])}}))
    #model.add(Activation('relu'))
    model.add(Activation({{choice(['relu','sigmoid','tanh','selu'])}}))
    model.add(BatchNormalization())
...
best_model.save('./models/model22.h5')

model=tf.keras.models.load_model('./models/model22.h5')
model.summary()
